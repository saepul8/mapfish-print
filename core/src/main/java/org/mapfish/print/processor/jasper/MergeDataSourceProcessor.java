 ...
 * configure the template with a detail section that contains a subreport, the name of which is a field in the
 * DataSources and the DataSources for the sub-template another field.  Then you can merge the legend and the
 * tables into a single DataSources.  This way the report will nicely expand depending on if you have a legend
 * and how many tables you have in your report.
 * </p>
...
public final class MergeDataSourceProcessor
    ...
    /**
     * An enumeration of the different <em>types</em> of source objects.  Essentially this describes how the
     * source should be merged into the final merged DataSource.
     */
    public enum SourceType {
        /**
         * Creates a single row from a set of values from the output and attribute objects.
         * <p>
         * In this case the key is not required, only the fields.  Each field key will be the look up key to
         * find the object from the set of processor output and attributes.  The field value will be the
         * column name for that value in the created row
         * </p>
         */
        SINGLE {
            @Override
         ...
         * </p>
         * <p>
         * The fields parameter of the source should contain all the fields to pull from the source
         * DataSource.  Not all Fields need to be declared.  For example if the source has 5 fields not all of
         * them need to be in the resulting merged datasource.
         * </p>
         */
        DATASOURCE {
            @Override
